		...
		elseif sample_alg[gx][sgx][:type] == :dep
			...
			function step_wrapper_sgdepvar(new_param)
				...
				if alg[sample_alg[gx][sgx][:alg]] isa MH						
					nw_param_val =[bk_param_val[1:sg_p_loc-1]...,new_param..., bk_param_val[sg_p_loc+length(new_param):end]...]
				else						
					ri_loc = 1
					for ri in 1:sample_alg[gx][sgx][:n_vars]
						nw_param_val[sg_p_loc+ri-1] = deepcopy(revt[sg_p_loc+ri-1](new_param[ri_loc:ri_loc+sample_alg[gx][sgx][ri][:n_eles]-1]))
